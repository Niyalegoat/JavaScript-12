...
if(discount > 0 && isHoliday){
    console.log("Apply holiday discounty.");
}

const agreedToTerms = fale;
const isHoliday=  true;
if(!agreeToTerm){
    console.log("You must agree to term first.");
}
...
function checkUserForm() {
    ...
    if (name !== "" && email !== "") {
        document.getElementById("forMessage").textContent = "Form complete!";
    }else{
        document.getElementById("formMessage").textContent = "Please fill inboth fields.";
    }
}

function checkDiscounty() {
    const checked =
...
function toggleNotice() {
    ...
    if (!notify) {
        document.getElementById("notifyBox").textContent = "Notifications are OFF";
    }else{
        document.getElementById("badgeMsg").textContent = "Notifications are ON";
    }
}